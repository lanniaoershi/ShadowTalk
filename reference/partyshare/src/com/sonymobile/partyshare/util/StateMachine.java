...
public class StateMachine {
    ...
    /**
     * Convenience constant that maybe returned by processMessage to indicate
    ...
    public static class LogRec {
         ...
         * @param orgState is the first state the received the message but did
         *            not processes the message.
         * @param transToState is the state that was transitioned to after the
         ...
         * @param dstState is the state that was the transition target when
         *            logging
         */
        public void update(StateMachine sm, Message msg, String info, IState state,
        ...
        /**
         * @return the state destination state if a transition is occurring or
         *         null if none.
         */
    ...
    /**
     * A list of log records including messages recently processed by the state
     * machine.
     *
     * The class maintains a list of log records including messages recently
     * processed. The list is finite and may be set in the constructor or by
     * calling setSize. The public interface also includes size which returns
     * the number of recent records, count which is the number of records
     * processed since the the last setSize, get which returns a record and add
     * which adds a record.
     */
    ...
    private static class SmHandler extends Handler {
        ...
        /**
         * A list of log records including messages this state machine has
         * processed
         */
        ...
        private void performTransitions(State msgProcessedState, Message msg) {
            ...
            /**
             * Record whether message needs to be logged before we transition
             * and and we won't log special messages SM_INIT_CMD or SM_QUIT_CMD
             * which always set msg.obj to the handler.
             */
            ...
            if (destState != null) {
                /**
                 * Process the transitions including transitions in the
                 * enter/exit methods
                 */
                while (true) {
                    if (mDbg)
                     ...
                     * queue so they will be processed before any other messages
        ...
        /**
         * Cleanup all the static variables and the looper after the SM has been
         * quit.
         */
    ...
    private void startHandlerThread(String name) {
    ...
    /**
     * Called for any message that is received after transitionToHalting is
     * called.
     */
    ...
    /**
     * This will be called once after a quit message that was NOT handled by the
    ...
    public final Collection<LogRec> copyLogRecs() {
        ...
        SmHandler smh = mSmHandler;
        if (smh != null) {
            for (int i = 0; i < smh.mLogRecords.size(); i++) {
                LogRec lr = smh.mLogRecords.get(i);
                vlr.add(lr);
            }
        }
        return vlr;
     ...
     * Override if additional information is desired.
     *
     ...
     *
     * Note: The handler can be null if the state machine has quit, which means
     * target will be null and may cause a AndroidRuntimeException in
     * MessageQueue#enqueMessage if sent directly or if sent using
    ...
    /**
     * Get a message and set Message.target state machine handler, what, arg1
     * and arg2
     *
     * Note: The handler can be null if the state machine has quit, which means
     * target will be null and may cause a AndroidRuntimeException in
    ...
    public final void sendMessageDelayed(
            int what, int arg1, int arg2, Object obj, long delayMillis) {
        // mSmHandler can be null if the state machine has quit.
        SmHandler smh = mSmHandler;
        if (smh == null)
            return;
    ...
    /**
     * Removes a message from the message queue. Protected, may only be called
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        for (int i = 0; i < getLogRecSize(); i++) {
            ...
            if (logRec != null) {
                pw.printf(" rec[%d]: %s%n", i, logRec.toString());
                pw.flush();
            }
        }
